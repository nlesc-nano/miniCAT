...
class HydrogenHalideFG(FGDef):
    """Hydrogen halides HX (X = Cl, Br, I) → halide anions X- (anion role).
    Neutral detection matches a halogen with one explicit/implicit H: [Cl,Br,I;H1].
    Anchor: center = halide atom; vector = +z (arbitrary for monoatomic).
    """
    neutral_pattern = Chem.MolFromSmarts("[Cl,Br,I;H1]")
    def __init__(self):
        super().__init__("halide", "anion", "[Cl,Br,I;H1]")
    def to_ionic(self, mol: Chem.Mol, match: Tuple[int, ...]) -> Chem.Mol:
        X_idx = match[0]
        X = mol.GetAtomWithIdx(X_idx)
        # Remove one attached hydrogen if present and set charge -1
        h_to_remove = None
        for nb in X.GetNeighbors():
            if nb.GetAtomicNum() == 1:
                h_to_remove = nb.GetIdx()
                break
        rw = Chem.RWMol(mol)
        if h_to_remove is not None:
            ...
            mol = rw.GetMol()
        mol.GetAtomWithIdx(X_idx).SetFormalCharge(-1)
        Chem.SanitizeMol(mol, sanitizeOps=Chem.SanitizeFlags.SANITIZE_ADJUSTHS)
        return mol
    def to_neutral(self, mol: Chem.Mol, match: Tuple[int, ...]) -> Chem.Mol:
        # X- + H → HX (add H back, neutralize)
        X_idx = match[0]
        X = mol.GetAtomWithIdx(X_idx)
        if X.GetFormalCharge() == -1:
            em = Chem.EditableMol(mol)
            H_idx = em.AddAtom(Chem.Atom(1))
            mol = em.GetMol()
            conf = mol.GetConformer()
            xpos = conf.GetAtomPosition(X_idx)
            conf.SetAtomPosition(H_idx, Point3D(xpos.x + 1.0, xpos.y, xpos.z))
            mol = Chem.RWMol(mol)
            mol.AddBond(X_idx, H_idx, Chem.BondType.SINGLE)
            mol = mol.GetMol()
            mol.GetAtomWithIdx(X_idx).SetFormalCharge(0)
            Chem.SanitizeMol(mol)
        return mol
    def anchor_center_and_vector(self, mol: Chem.Mol, match: Tuple[int, ...]) -> Tuple[np.ndarray, np.ndarray]:
        coords = rdconf_to_numpy(mol)
        X_idx = match[0]
        Xpos = coords[X_idx]
        return Xpos, np.array([0.0, 0.0, 1.0])

class HydrogenSulfideFG(FGDef):
    """Hydrogen sulfide H2S → hydrosulfide HS- (anion role).
    Neutral detection: [SH2]. On deprotonation remove one H and set S:-1.
    Anchor: center = S; vector = +z if no heavy-atom neighbors.
    """
    neutral_pattern = Chem.MolFromSmarts("[SH2]")
    def __init__(self):
        super().__init__("hydrosulfide", "anion", "[SH2]")
    def to_ionic(self, mol: Chem.Mol, match: Tuple[int, ...]) -> Chem.Mol:
        S_idx = match[0]
        S = mol.GetAtomWithIdx(S_idx)
        # Find a hydrogen bound to S and remove one
        h_to_remove = None
        for nb in S.GetNeighbors():
            if nb.GetAtomicNum() == 1:
                h_to_remove = nb.GetIdx()
    ...
    def anchor_center_and_vector(self, mol: Chem.Mol, match: Tuple[int, ...]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        S_idx = match[0]
        Spos = coords[S_idx]
        # Try to orient along non-H neighbor if exists
        neighs = [n.GetIdx() for n in mol.GetAtomWithIdx(S_idx).GetNeighbors() if mol.GetAtomWithIdx(n.GetIdx()).GetAtomicNum() != 1]
        if neighs:
            vec = coords[neighs[0]] - Spos
            if np.linalg.norm(vec) > 1e-8:
                return Spos, unit(vec)
        return Spos, np.array([0.0, 0.0, 1.0])
...
def detect_fg_matches_neutral(mol: Chem.Mol, role: str, registry: Optional[List[FGDef]] = None
    ...
    for fg in registry:
        ...
        elif isinstance(fg, HydrogenHalideFG):
            patt = HydrogenHalideFG.neutral_pattern
        elif isinstance(fg, HydrogenSulfideFG):
