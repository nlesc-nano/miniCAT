...
# --- Simple acid SMILES sanitizer: map HX/H2S forms to explicit neutral molecules ---
def _sanitize_simple_acid_smiles(smiles: str) -> str:
    s = smiles.strip()
    s_upper = s.upper()
    # Accept 'HCl', 'HBR', 'HI' (with or without brackets), or bare 'Cl','Br','I'
    if s_upper in ('HCL', 'CL', '[H]CL', '[CLH]'):
        return '[H]Cl'
    if s_upper in ('HBR', 'BR', '[H]BR', '[BRH]'):
        return '[H]Br'
    if s_upper in ('HI', 'I', '[H]I', '[IH]'):
        return '[H]I'
    # H2S can be written as 'S' in SMILES; leave as 'S' (implicit H2)
    return s
import numpy as np
...
def _get_spatially_ordered_indices(dummy_pos: np.ndarray, maximize_dist: bool) -> List[int]:
    ...
    first_site_idx = op(total_proximities)
